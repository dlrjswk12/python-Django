...
public class UserController implements HttpSessionListener{
	private static final int LIMITED_RENT_BOOK_CNT = 4; //최대 대출책수
	private static final int LIMITED_RESERVE_BOOK_CNT = 3; //최대 예약책수
	...
	public String login(@ModelAttribute UserBean bean, HttpServletRequest request){
		...
		return "redirect:/work/book/goMain.do";
	}
	...
	@RequestMapping(value="/work/user/retrieveReserveBookCnt.do", method=RequestMethod.GET)
	@ResponseBody
	public ResponseEntity<Map<String, String>> retrieveReserveBookCnt(HttpServletRequest request){
		String checkMsg = "";
		HttpSession session = request.getSession();
		...
		resultParam = userService.retrieveReserveBookCnt(userParam);

		int reserveBookCnt = Integer.parseInt(String.valueOf(resultParam.get("RESERVE_BOOK_CNT")));
		if(reserveBookCnt >= 0 && reserveBookCnt < LIMITED_RESERVE_BOOK_CNT){
			checkMsg = "success";
		}else{
			checkMsg = "fail";
		}
